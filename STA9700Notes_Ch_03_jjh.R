...
plot(Man.Hours ~ Lot.Size, data = Westwood, 
     xlab = "lot sizes", ylab = "man-hours")
...
plot(residuals(m) ~ fitted.values(m),
...
library(ALSM)
normal.cor.test(residuals(m), sigma(m)^2) 
# obtain correlation coefficient between the ordered residuals and their expected values under normality; can use Table B.6 to obtain critical value to ascertain whether the plausibility of normality assumption. 
...
plot(Ridership ~ Maps, 
     data = Riders, xlab = "Maps distributed (thousands)",
     ylab="Increase in ridership (thousands)")

m.Riders<-lm(Ridership~Maps,data=Riders) 

summary(m.Riders)

plot(m.Riders$residuals ~ Riders$Maps,
     xlab = "Maps distributed (thousands)",
     ylab = "Residual")
